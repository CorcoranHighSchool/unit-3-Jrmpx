 ...
 */
func calulateArea(x : Double, y :Double) -> Double?{
    //don't allow for negative values
    guard x > 0 && y > 0 else {
        print("One or more of the values given are less then 0")
        return nil
    }
    
    return x * y
}
calulateArea(x: 3, y: 3)
calulateArea(x: -3, y: 3)
...
func add(alpha : Int?, beta : Int?) -> Int?{
    //vaildate that all values are not nill
...
var dude = createUser()
if let user = dude{
    print("The user's name is \(user.firstName)\(user.lastName), and they \(user.age) years old ")
}
